...
t_heap* ConnectionRouter<Heap>::timing_driven_route_connection_from_heap(int sink_node,
	...
	int corridor_index = cheapest->corridor_index;
        ...
        VTR_LOGV_DEBUG(router_debug_, "  Popping node %d (cost: %g) hist: (%f) pres_cost: (%f) (%s) corridor index: %d\n",
                            inode, cheapest->cost, get_single_rr_cong_acc_cost(inode),  get_single_rr_cong_pres_cost(inode, cost_params.pres_fac), describe_rr_node(device_ctx.rr_graph, device_ctx.grid, device_ctx.rr_indexed_data, inode, is_flat_).c_str(), corridor_index);
	...
	if (cost_params.detailed_router == 1 && cost_params.intra_tile_connection == false && cost_params.leak == true && corridor_index == std::numeric_limits<int>::max()){
	      // this condition ensures that once a node outside the allowed nodes is allowed, from that point on, all nodes as an extension of this node would be considered outside
	      next_corridor_index =  std::numeric_limits<int>::max();
	      // what about the case when corridor index is not max?
	      // cases when a long whcih is part of multiple corridors is still considered out of corridor. Relaxing this would lead to dissolving constraint of branch points
	      // change: do not check corridor list when pushing
	      // change: same goes when moving PRT to heap, for max corridor index do not query list of corridors
	}
	else if (cost_params.detailed_router == 1 && (node_type == OPIN || node_type == SOURCE || node_type == IPIN || node_type == SINK)) {
                next_corridor_index = corridor_index+1;
	}
	else if (cost_params.detailed_router == 1 && cost_params.intra_tile_connection == true) {
	      next_corridor_index =  std::numeric_limits<int>::max();
	}
	    ...
	    short corridor_yto = corridor.to_y;
	    ...
	    int inode_xlow = rr_graph_->node_xlow(RRNodeId(inode));
    	    int inode_ylow = rr_graph_->node_ylow(RRNodeId(inode));
	    ...
	    VTR_LOGV_DEBUG(router_debug_, "       pseg: low (%d, %d) high (%d, %d)\n", inode_xlow, inode_ylow, inode_xhigh, inode_yhigh);
	        ...
	        if (direction == Direction::INC) {
	    	    if(inode_xhigh < corridor.to_x){
	                next_corridor_index = corridor_index;
			//VTR_LOG("pop 1\n");
	    	    }
	    	    else {
	    	        //VTR_ASSERT(inode_xhigh == corridor.to_x);
	                next_corridor_index = corridor_index+1;
			//VTR_LOG("pop 2\n");
	    	    }
	        }	
	        else { //DEC
	    	    if(inode_xlow > corridor.to_x + 1){
                        next_corridor_index = corridor_index;
			//VTR_LOG("pop 3\n");
                    }
		    else {
	        ...
	        Direction direction = rr_graph_->node_direction(RRNodeId(inode));
                if (direction == Direction::INC) {
                    if(inode_yhigh < corridor.to_y){
                        next_corridor_index = corridor_index;
			//VTR_LOG("pop 5\n");
                    }
                    else {
                        //VTR_ASSERT(inode_yhigh == corridor.to_y);
                        next_corridor_index = corridor_index+1;
			//VTR_LOG("pop 6\n");
                    }
                }
                else { //DEC
                    if(inode_ylow > corridor.to_y + 1){
                        next_corridor_index = corridor_index;
			//VTR_LOG("pop 7\n");
                    }
		    else {
...
void ConnectionRouter<Heap>::timing_driven_expand_neighbour(t_heap* current,
    ...
    // 1. Leak mode and outside Steiner tree
    if (cost_params.detailed_router == 1 
                ...
                && ((size_t)corridor_index < corridor_data.corridors_per_connection.size())) 
    {
	    ...
	    int node_dx = to_xhigh - to_xlow;
	    int node_dy = to_yhigh - to_ylow;

	    t_rr_type corridor_type = (dx == 0) ? CHANY : CHANX;
	    
	    // even for dx = 0 or dy = 0, assign a direction although the direction has no meaning in this case because the corridor is not in that channel
	    // doing the below only to remove if else condition, which entails branch prediction
	    ...
	    if (corridor_type == node_type && corridor_direction == node_direction && const_coord_corridor == const_coord_node) {
            ...
            offpath_penalty = (node_direction == Direction::INC) ? 
                ((node_coord_comp_corr_to <= corridor_comp_coord_to && node_coord_comp_corr_from >= corridor_comp_coord_from) ? 1.0 : -1.0) 
                : ((node_coord_comp_corr_to >= corridor_comp_coord_to && node_coord_comp_corr_from <= corridor_comp_coord_from) ? 1.0 : -1.0); 
...
void ConnectionRouter<Heap>::timing_driven_add_to_heap(const t_conn_cost_params cost_params,
    ...
    VTR_LOGV_DEBUG(router_debug_, "      Expanding to node %d cost: (%f) back_cost: (%f) hist: (%f) pres_cost: (%f) (%s)\n", to_node, new_total_cost, new_back_cost, get_single_rr_cong_acc_cost(to_node),  get_single_rr_cong_pres_cost(to_node, cost_params.pres_fac), describe_rr_node(device_ctx.rr_graph, device_ctx.grid, device_ctx.rr_indexed_data, to_node, is_flat_).c_str());
    if (new_total_cost < best_total_cost && ((rcv_path_manager.is_enabled()) || (new_back_cost < best_back_cost))) {
    	...
    	//SHA: pass the corridor_index, necessary when routing along the coarse regions
        next_ptr->corridor_index = corridor_index;
...
void ConnectionRouter<Heap>::evaluate_timing_driven_node_costs(t_heap* to,
	...
	if (cost_params.detailed_router != 1  || cost_params.leak == true || cost_params.intra_tile_connection == true) {
            ...
            /* Experiment 1 and 2 (E1 / E2) : apply bias only to the lookahead inside coarse region */
            //VTR_LOG("lookahead was %f, and now applying offpath penalty %f\n", expected_cost, offpath_penalty);
            //expected_cost *= offpath_penalty;
	} else {
...
//Adds only the branch node of route tree rooted at rt_node to the heap, preparing it to be
//used as branch-points for further routing.
template<typename Heap>
bool ConnectionRouter<Heap>::add_only_branch_node_of_route_tree_to_heap(
    t_rt_node* rt_node,
    int target_node,
    const t_conn_cost_params cost_params,
    std::set<int> branch_nodes, int seg_index_branch_node) {
    ...
    while (linked_rt_edge != nullptr) {
        ...
        bool found = add_only_branch_node_of_route_tree_to_heap(child_node, target_node,
...
void ConnectionRouter<Heap>::add_route_tree_to_heap(
    ...
    if (rt_node->re_expand) {
	if (cost_params.detailed_router == 1 && cost_params.intra_tile_connection == false) {	
		int to_node = rt_node->inode;
		corridor_index = rt_node->corridor_index;
		//if (sink_id == 14849598) {
        	//VTR_LOGV_DEBUG(router_debug_, "  Checking node %8d to heap from init route tree (%s) corridor index: %d\n", to_node,
                 //      describe_rr_node(device_ctx.rr_graph, device_ctx.grid, device_ctx.rr_indexed_data, to_node, is_flat_).c_str(), corridor_index);
		//}
		...
		if (corridor_index < 0) {
		    ...
		    int eff_corridor_from_y = corridor.from_y;
	            ...
	            Direction corridor_direction = (dx != 0) ? (dx > 0 ? Direction::INC : Direction::DEC)
                                			     : (dy > 0 ? Direction::INC : Direction::DEC);
	            // update "to" corridor coordinate for DEC direction 
	...
	else if (cost_params.detailed_router == 0 || cost_params.intra_tile_connection == true){
	    int to_node = rt_node->inode;
            corridor_index = rt_node->corridor_index;
...
//Unconditionally adds rt_node to the heap with zero cost
//
//Note that if you want to respect rt_node->re_expand that is the caller's
//responsibility.
template<typename Heap>
void ConnectionRouter<Heap>::add_route_tree_node_to_heap_with_zero_cost(
    t_rt_node* rt_node,
    int target_node,
    const t_conn_cost_params cost_params) {
    const auto& device_ctx = g_vpr_ctx.device();
    ...
    if (!rcv_path_manager.is_enabled()) {
        ...
        float tot_cost = 0.;
	int corridor_index;
        VTR_LOGV_DEBUG(router_debug_, "  Adding node %8d to heap from init route tree with cost %g (%s) corridor index: %d\n", inode, tot_cost,
        ...
        push_back_node(&heap_, rr_node_route_inf_,
                       ...
                       backward_path_cost, R_upstream, corridor_index);
    } else {
...
void ConnectionRouter<Heap>::add_route_tree_node_to_heap(
	...
	// have a list storing distance to source from each corridor
	// of course when a wire of length smaller than corridor length is used, then the cost
...
template<typename Heap>
t_bb ConnectionRouter<Heap>::add_only_branch_node_of_high_fanout_route_tree_to_heap(
    t_rt_node* rt_root,
    int target_node,
    const t_conn_cost_params cost_params,
    const SpatialRouteTreeLookup& spatial_rt_lookup,
    ...
    for (int dx : {0, -1, +1}) {
		...
		if (seg_index_node == seg_index_branch_node){
				...
				int corridor_index = 0;
                		add_route_tree_node_to_heap_with_zero_cost(rt_node, target_node, cost_params);
