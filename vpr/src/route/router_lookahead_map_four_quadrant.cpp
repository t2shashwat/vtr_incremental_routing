...
static void run_dijkstra(RRNodeId start_node, int start_x, int start_y, int quadrant, t_routing_cost_map& routing_cost_map, t_dijkstra_data* data);
...
std::pair<float, float> MapLookahead::get_expected_delay_and_cong(RRNodeId from_node, RRNodeId to_node, const t_conn_cost_params& params, float /*R_upstream*/) const {
    ...
    if (delta_x >= 0 && delta_y <= 0){
        ...
        offset_x = device_ctx.grid.width();
        offset_y = 0;
    }
    else if (delta_x < 0 && delta_y < 0){
        offset_x = device_ctx.grid.width();
        offset_y = device_ctx.grid.height();
    }
    else if (delta_x > 0 && delta_y < 0){
        offset_x = 0;
        offset_y = device_ctx.grid.height();
    ...
    int og_delta_x = delta_x; 
    int og_delta_y = delta_y; 
    delta_x = abs(delta_x) + offset_x;
    delta_y = abs(delta_y) + offset_y;
    ...
    if (from_type == SOURCE || from_type == OPIN) {
        ...
        if (this->src_opin_delays[tile_index][from_ptc].empty()) {
            ...
            if (og_delta_x < 0 && og_delta_y < 0)
                VTR_LOG("Empty src_opin_delays  %f %f\n", expected_delay_cost, expected_cong_cost);
    ...
    if (og_delta_x < 0 && og_delta_y < 0)
        VTR_LOG("seg: %d chan_index: %d LA value: og: (%d, %d) offset: (%d, %d)  %f %f\n", seg_value, chan_index, og_delta_x, og_delta_y, delta_x, delta_y, expected_delay_cost, expected_cong_cost);
...
static void compute_router_wire_lookahead(const std::vector<t_segment_inf>& segment_inf, const float sbNode_lookahead_factor) {
    ...
    // increased lookahead size by 4x
    f_wire_cost_map = t_wire_cost_map({2, segment_inf.size(), 2*device_ctx.grid.width(), 2*device_ctx.grid.height()});
    ...
    int q1_target_x = device_ctx.grid.width() - 2;
    int q1_target_y = device_ctx.grid.height() - 2;
    // for quadrant 2
    int q2_ref_x = device_ctx.grid.width() - 2;
    int q2_ref_y = 1;
    // Upper left non-corner
    int q2_target_x = 1;
    int q2_target_y = device_ctx.grid.height() - 2;
    // for quadrant 3
    int q3_ref_x = device_ctx.grid.width() - 2;
    int q3_ref_y = device_ctx.grid.height() - 2;
    // Lower left non-corner
    int q3_target_x = 1;
    int q3_target_y = 1;
    // for quadrant 4
    int q4_ref_x = 1;
    int q4_ref_y = device_ctx.grid.height() - 2;
    // Lower right non-corner
    int q4_target_x = device_ctx.grid.width() - 2;
    int q4_target_y = 1;
    
    std::vector<std::pair<int, int>> ref_coordinates = {{q1_ref_x, q1_ref_y}, {q2_ref_x, q2_ref_y},
                                       {q3_ref_x, q3_ref_y}, {q4_ref_x, q4_ref_y}};
    
    std::vector<std::pair<int, int>> target_coordinates = {{q1_target_x, q1_target_y}, {q2_target_x, q2_target_y},
                                       {q3_target_x, q3_target_y}, {q4_target_x, q4_target_y}};
    
    std::vector<std::pair<int, int>> q1_sample_coordinates = {{q1_ref_x, q1_ref_y}, 
                                                              {q1_ref_x + 1, q1_ref_y + 1},
                                                              {q1_ref_x + longest_length, q1_ref_y + longest_length}, 
                                                              {q1_ref_x + longest_length + 1, q1_ref_y + longest_length + 1}};
    std::vector<std::pair<int, int>> q2_sample_coordinates = {{q2_ref_x, q2_ref_y}, 
                                                              {q2_ref_x - 1, q2_ref_y + 1},
                                                              {q2_ref_x - longest_length, q2_ref_y + longest_length}, 
                                                              {q2_ref_x - longest_length - 1, q2_ref_y + longest_length + 1}};
    std::vector<std::pair<int, int>> q3_sample_coordinates = {{q3_ref_x, q3_ref_y}, 
                                                              {q3_ref_x - 1, q3_ref_y - 1},
                                                              {q3_ref_x - longest_length, q3_ref_y - longest_length}, 
                                                              {q3_ref_x - longest_length - 1, q3_ref_y - longest_length - 1}};
    std::vector<std::pair<int, int>> q4_sample_coordinates = {{q4_ref_x, q4_ref_y}, 
                                                              {q4_ref_x + 1, q4_ref_y - 1},
                                                              {q4_ref_x + longest_length, q4_ref_y - longest_length}, 
                                                              {q4_ref_x + longest_length + 1, q4_ref_y - longest_length - 1}};

    std::vector<std::vector<std::pair<int, int>>> all_coordinates = {
        q1_sample_coordinates,
        q2_sample_coordinates,
        q3_sample_coordinates,
        q4_sample_coordinates
    };
    ...
    int grid_width = device_ctx.grid.width();
    int grid_height = device_ctx.grid.height();
    for (int iseg = 0; iseg < int(segment_inf.size()); iseg++) {
        ...
        for (e_rr_type chan_type : chan_types) {
            for (size_t i = 0; i < all_coordinates.size(); i++){
                const auto& quadrant_coordinates = all_coordinates[i];
                const auto& target = target_coordinates[i];
                int target_x = target.first;
                int target_y = target.second;
                VTR_LOG("%d %d --> %d %d\n", quadrant_coordinates[0].first, quadrant_coordinates[0].second, target_x, target_y);
                
                for (const auto& coordinate : quadrant_coordinates){
                    int sample_x = coordinate.first;
                    int sample_y = coordinate.second;
        ...
        //increased the size by 4x
        t_routing_cost_map routing_cost_map({2*device_ctx.grid.width(), 2*device_ctx.grid.height()});
        int quadrant;
        for (e_rr_type chan_type : chan_types) {
            if (sample_nodes[chan_type].empty()) {
                ...
                for (RRNodeId sample_node : sample_nodes[chan_type]) {
                    ...
                    if (sample_x < grid_width/2 && sample_y < grid_height/2) {
                        quadrant = 1; // Quadrant I
                    } else if (sample_x > grid_width/2 && sample_y < grid_height/2) {
                        quadrant = 2; // Quadrant II
                    } else if (sample_x > grid_width/2 && sample_y > grid_height/2) {
                        quadrant = 3; // Quadrant III
                    } else if (sample_x < grid_width/2 && sample_y > grid_height/2) {
                        quadrant = 4; // Quadrant IV
                    } else {
                        VTR_ASSERT("Wrong sampling of nodes for router lookahead\n");
                    } 
...
static void run_dijkstra(RRNodeId start_node, int start_x, int start_y, int quadrant, t_routing_cost_map& routing_cost_map, t_dijkstra_data* data) {
    auto& device_ctx = g_vpr_ctx.device();
    const auto& rr_graph = device_ctx.rr_graph;
    int offset_x = device_ctx.grid.width();
    int offset_y = device_ctx.grid.height();
    ...
    while (!pq.empty()) {
        ...
        if (rr_graph.node_type(curr_node) == IPIN) {
            ...
            if ((quadrant == 1) && (ipin_x >= start_x && ipin_y >= start_y)) {
                int delta_x, delta_y;
            ...
            else if ((quadrant == 2) && (ipin_x <= start_x && ipin_y >= start_y)) {
                int delta_x, delta_y;
                get_xy_deltas(start_node, curr_node, &delta_x, &delta_y);
                delta_x = std::abs(delta_x) + offset_x;
                delta_y = std::abs(delta_y);

                routing_cost_map[delta_x][delta_y].add_cost_entry(current.delay, current.congestion_upstream);
            }
            else if ((quadrant == 3) && (ipin_x <= start_x && ipin_y <= start_y)) {
                int delta_x, delta_y;
                get_xy_deltas(start_node, curr_node, &delta_x, &delta_y);
                delta_x = std::abs(delta_x) + offset_x;
                delta_y = std::abs(delta_y) + offset_y;

                routing_cost_map[delta_x][delta_y].add_cost_entry(current.delay, current.congestion_upstream);
            }
            else if ((quadrant == 4) && (ipin_x >= start_x && ipin_y <= start_y)) {
                int delta_x, delta_y;
                get_xy_deltas(start_node, curr_node, &delta_x, &delta_y);
                delta_x = std::abs(delta_x);
                delta_y = std::abs(delta_y) + offset_y;
...
static void fill_in_missing_lookahead_entries(int segment_index, e_rr_type chan_type) {
    ...
    for (unsigned ix = 0; ix < 2*device_ctx.grid.width(); ix++) {
        for (unsigned iy = 0; iy < 2*device_ctx.grid.height(); iy++) {
...
void read_router_lookahead(const std::string& file) {
    ...
    VTR_LOG("Values inside the loaded router lookahead:\n");
    VTR_LOG("Dimensions of lookahead: %d %d\n", f_wire_cost_map.dim_size(2), f_wire_cost_map.dim_size(3));
    int chan_index = 0;//CHANX
    int segment_index = 0; //seg1
    for (size_t x = 0; x < f_wire_cost_map.dim_size(2); x++) {
        for (size_t y = 0; y < f_wire_cost_map.dim_size(3); y++) {
            //for (size_t seg_id = 0; seg_id < 4;)
            Cost_Entry entry = f_wire_cost_map[chan_index][segment_index][x][y];
            VTR_LOG(" (%zu,%zu) --> delay=%10.3g cong=%10.3g\n", x, y, entry.delay, entry.congestion);
        }
    }
}
